...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/06/12 18:55:29 by cdohanic          #+#    #+#             */
/*   Updated: 2026/06/12 18:55:31 by cdohanic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt_bonus.h"

static void	set_viewport(t_data *data, cl_float3 w, cl_float3 u, cl_float3 v)
{
	cl_float3	ulc;

	data->frame.horizontal = scale(u, data->frame.viewport_width);
	data->frame.vertical = scale(v, -data->frame.viewport_height);
	data->frame.pixel_delta_u = scale(data->frame.horizontal,
			1.0f / (float)data->frame.width);
	data->frame.pixel_delta_v = scale(data->frame.vertical,
			1.0f / (float)data->frame.height);
...
void	update_view(t_data *data)
{
	...
	u = norm(cross(vup, w));
	v = cross(w, u);
	set_viewport(data, w, u, v);
}

/*
** Build the camera basis (u, v, w) from cam_dir + fov and derive the viewport
** vectors. Ported from initialize() in mandatory/src/main_cpu.c.
*/
